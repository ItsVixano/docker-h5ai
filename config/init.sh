...
if [ "$( grep -rni "$TZ" /etc/php83/conf.d/00_timezone.ini | wc -l )" -eq 0 ]; then
    msg "Configure timezone for PHP..."
    echo "$TZ\"" >> /etc/php83/conf.d/00_timezone.ini
fi
...
sed -i "s#user = nobody.*#user = abc#g" /etc/php83/php-fpm.d/www.conf
sed -i "s#group = nobody.*#group = abc#g" /etc/php83/php-fpm.d/www.conf
